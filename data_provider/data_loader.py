...
class SIUloader(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='siu.csv',
                 target='OT', scale=True, timeenc=0, freq='h', seasonal_patterns=None):
        pass
        # size [seq_len, label_len, pred_len]
        self.args = args
        # info
        if size == None:
            # default is: 24hr in, 1hr out
            self.seq_len = 24   # encoder context len
            self.label_len = 24 # input len
            self.pred_len = 1   # prediction len
        else:
            self.seq_len = size[0]
            self.label_len = size[1]
            self.pred_len = size[2]
        ...
        self.features = features # mutli of single variate
        self.target = target     # feature to predict
        self.scale = scale
        self.timeenc = timeenc
        self.freq = freq
    ...
    def __read_data__(self):
        ...
        border1s = np.array([0,             n_samples*0.8 - self.seq_len,  n_samples*0.9 - self.seq_len], dtype=int) # note: this doesn't do a random split
        border2s = np.array([n_samples*0.8, n_samples*0.9,                 n_samples], dtype=int)
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]
